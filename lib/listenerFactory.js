...
Listener.prototype.processRecord = function(record){
  var listener = this;
  slackClient.postAsync(listener.slackWebhookUri, listener.transform(record))
...
Listener.prototype.start = function() {
  var listener = this;
  listener.stream = listener.consumer.subscribe(listener.topic);
  listener.stream.on('read', function(msgs){
    listener.log.info(msg)
    _.forEach(_.pluck(msgs, 'value'), listener.processRecord.bind(listener));
  });
